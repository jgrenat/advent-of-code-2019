...
parse value =
    String.lines value
        ...
        |> List.map (List.filter (String.isBlank >> not))
        |> List.map (List.map parseDirection)
        |> List.filter (List.isEmpty >> not)
        |> toTuple


parseDirection : String -> Direction
parseDirection positionString =
    case String.toList positionString of
        letter :: numbers ->
            case ( letter, String.toInt (String.fromList numbers) ) of
                ( _, Nothing ) ->
                    Debug.todo "Invalid numbers !!!"
...
toTuple : List (List Direction) -> ( List Direction, List Direction )
toTuple positions =
    case positions of
        (firstPosition1 :: others1) :: (firstPosition2 :: others2) :: _ ->
            ( firstPosition1 :: others1, firstPosition2 :: others2 )
...
type alias Accumulator =
    { currentOrder : Int
    , orders : Dict Position Int
    , currentPosition : Position
    , positions : Set Position
    }


computePositions : List Direction -> ( Set Position, Dict Position Int )
computePositions directions =
    List.foldl
        (\direction { currentOrder, orders, currentPosition, positions } ->
            let
                ( lastX, lastY ) =
                    currentPosition

                ( newOrder, newPositions, newOrders ) =
                    case direction of
                        Up value ->
                            ( currentOrder + value
                            , List.range 1 value
                                |> List.map (\increment -> ( lastX + increment, lastY ))
                            , List.range 1 value
                                |> List.foldl (\increment -> Dict.insert ( lastX + increment, lastY ) (currentOrder + increment)) Dict.empty
                            )
                        ...
                        Left value ->
                            ( currentOrder + value
                            , List.range 1 value
                                |> List.map (\increment -> ( lastX, lastY - increment ))
                            , List.range 1 value
                                |> List.foldl (\increment -> Dict.insert ( lastX, lastY - increment ) (currentOrder + increment)) Dict.empty
                            )
            ...
            in
            Accumulator newOrder (Dict.union orders newOrders) (List.last newPositions |> Maybe.withDefault ( 0, 0 )) (Set.union positions (Set.fromList newPositions))
        )
        (Accumulator 0 Dict.empty ( 0, 0 ) Set.empty)
        directions
        |> (\accumulator -> ( accumulator.positions, accumulator.orders ))
...
main =
    parse input
        |> Tuple.mapBoth computePositions computePositions
        |> (\( ( positions1, orders1 ), ( positions2, orders2 ) ) -> ( Set.toList (Set.intersect positions1 positions2), orders1, orders2 ))
        |> (\( collisions, orders1, orders2 ) ->
                List.filterMap
                    (\position ->
                        Maybe.map2 (+) (Dict.get position orders1) (Dict.get position orders2)
                    )
                    collisions
           )
        |> List.minimum
